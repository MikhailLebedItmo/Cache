...
int main(int argc, char** argv) {
    ...
    cmd_args_parser.parse_all_arguments();
    auto [replacement, asm_file_path, bin_file_path] = cmd_args_parser.get_arguments();
    AsmCommandParser asm_command_parser(asm_file_path);
    asm_command_parser.parse_all_commands();
    auto commands = asm_command_parser.get_commands();
    if (replacement == 0 || replacement == 1) {
        LRUCache cache(CACHE_WAY, CACHE_TAG_LEN, CACHE_INDEX_LEN, CACHE_OFFSET_LEN);
